            ...
            <table>
              ...
              <tbody>
                <tr v-for="product in dashboardData?.top_products" :key="product.id">
                  ...
                  <td>{{ product.moq_count || 0 }}</td>
                  <td>KES {{ (product.price * (product.moq_count || 0)).toFixed(2) }}</td>
                  <td>
...
export default {
  ...
  setup() {
    ...
    return { dashboardData, revenueChartData, salesByLocationData, totalSalesData, chartOptions, error };
  },
};
</script>
