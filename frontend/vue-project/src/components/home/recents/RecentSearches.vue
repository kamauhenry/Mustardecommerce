...
export default {
  setup() {
    ...
    const fetchRandomProducts = async () => {
      try {
        const response = await fetch('http://127.0.0.1:8000//api/products/random/?limit=3');
    ...
    const fetchProductForSearch = async (query) => {
      try {
        const response = await fetch(
          `http://127.0.0.1:8000//api/products/search/?search=${encodeURIComponent(query)}&page=1&per_page=1`
        );
