...
class BulkUpdateOrderStatusTest(TestCase):
    def setUp(self):
        self.admin = User.objects.create_user(
            username='mustardimports', password='mustard1q2w3e4r', user_type='admin'
        )
        self.user = User.objects.create_user(
            username='testuser', email='test@example.com', password='testpass'
        )
...
class AuthenticationTest(TestCase):
    ...
    def test_google_auth(self, mock_verify):
        ...
        response = self.client.post(reverse('google_auth'), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn('token', response.data)
...
class PaymentTest(TestCase):
    ...
    @patch('ecommerce.views.query_stk_push')
...
class ProductCategoryTest(TestCase):
    ...
    def test_get_product_details(self):
        response = self.client.get(reverse('product_detail', args=['test-category', 'test-product']))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['name'], 'Test Product')

    def test_get_category_products(self):
        response = self.client.get(reverse('category_products', args=['test-category']))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class UserProfileDeliveryTest(TestCase):
    ...
    def test_create_delivery_location(self):
        ...
        response = self.client.post(reverse('delivery_location'), data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
